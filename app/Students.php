...
//    }
    ...
    public function doPostStudent($request, $user) {
      ...
      //  } else {
       ...
       // }
        $uname = $request['registration_no'].'s';
       // dd($uname);
        ...
        if ($student_id) {
            ...
            if ($pId) {
              ...
              //  } else {
               ...
               // }
                $p_unq_id = $request['registration_no'].'p';
                // $p_unq_id = $fname.$pId."p";
               // dd($p_unq_id);
                $parent_id = \DB::table('users')->insertGetId([
                    'type' => 'parent',
    ...
    public function doImportMapping($user, $request){
      ...
      //dd($request);
        ...
        $objPHPExcel = \PHPExcel_IOFactory::load($request['file']);
        $obj = $objPHPExcel->getActiveSheet();
        if ($obj->getCellByColumnAndRow(0, 11)->getValue() != 'session_id' &&
                $obj->getCellByColumnAndRow(1, 11)->getValue() != 'class_id' &&
                $obj->getCellByColumnAndRow(2, 11)->getValue() != 'registration_no' &&
                $obj->getCellByColumnAndRow(3, 11)->getValue() != 'bus_id' &&
                $obj->getCellByColumnAndRow(4, 11)->getValue() != 'route_id' &&
                $obj->getCellByColumnAndRow(5, 11)->getValue() != 'boarding_id ' &&
                $obj->getCellByColumnAndRow(6, 11)->getValue() != 'fee_name '){
        ...
        $Iterator = 0;
        $ins_count=0;
    for (((($obj->getCellByColumnAndRow(0, $row)->getValue()) == 'session_id') ? $row = 12 : $row = 11); $row <= $rows; ++$row) {
            $Accounts[$Iterator] = array(
                'session_id' => $obj->getCellByColumnAndRow(0, $row)->getValue(),
                'class_id' => $obj->getCellByColumnAndRow(1, $row)->getValue(),
                'registration_no' => $obj->getCellByColumnAndRow(2, $row)->getValue(),
                'bus_id' => $obj->getCellByColumnAndRow(3, $row)->getValue(),
                'route_id' => $obj->getCellByColumnAndRow(4, $row)->getValue(),
                'boarding_id' => $obj->getCellByColumnAndRow(5, $row)->getValue(),
                'fee_name' => $obj->getCellByColumnAndRow(6, $row)->getValue()
            ...
            foreach ($Accounts as $key => $val) {
                ...
                //Route Exist check 
                $route_no_exist = \DB::table('boarding')->where('school_id', \Auth::user()->school_id)->where('session_id', $val['session_id'])->where('route_id', $val['route_id'])->first();
                //echo $route_exist;
                ...
                //check boarding
                $boarding_no_exist = \DB::table('boarding')->where('school_id', \Auth::user()->school_id)->where('session_id', $val['session_id'])->where('route_id', $val['route_id'])->where('id', $val['boarding_id'])->first();
                //echo $route_exist;
                ...
                $busFees = \DB::table('boarding')->where('school_id', \Auth::user()->school_id)->where('session_id', $val['session_id'])->where('route_id', $val['route_id'])->where('id', $val['boarding_id'])->get();
               ...
               /* $registration_exist = \DB::table('sionfee_structure')->where('school_id', \Auth::user()->school_id)->where('reg_no', $val['registration_no'])->first();
                ...
                $routename = \DB::table('route_details')->where('school_id', \Auth::user()->school_id)->where('id', $val['route_id'])->first();
                $bus_nos = \DB::table('busdetails')->where('school_id', \Auth::user()->school_id)->where('id', $val['bus_id'])->first();
            ...
            if ($val['registration_no']) {
                ...
                $boarding=$val['boarding'];
                 ...
                 /*  $stu_boarding= \DB::table('boarding')->insertGetId([
                'school_id' => \Auth::user()->school_id,
               'session_id'=> (int) $val['session_id'],
               'route'=> $val['route'],
               'bus_no'=> $val['bus_no'],
               ...
               'bus_fee'=> $bus_amt,
                    ...
                    $stu_feeid=\DB::table('sionfee_structure')->insertGetId([
                        ...
                        'board_id' => $val['boarding_id'],
                        'bus_id'=>$val['bus_id'],
                        'route_id'=>$val['route_id'],
                        'boarding' => $boarding_no_exist->boarding,
                        'bus_no'=>$bus_nos->bus_no,
                        'bus_route'=>$routename->route_name,
                        'amount' => $bus_amt,
                        'fees_name' => $val['fee_name'],
    ...
    public function doImportStudentFees($user, $request){
        ...
        if ($obj->getCellByColumnAndRow(0, 11)->getValue() != 'session_id' &&
                $obj->getCellByColumnAndRow(1, 11)->getValue() != 'class_id' &&
                $obj->getCellByColumnAndRow(2, 11)->getValue() != 'registration_no' &&
                $obj->getCellByColumnAndRow(3, 11)->getValue() != 'payment_type1' &&
                $obj->getCellByColumnAndRow(4, 11)->getValue() != 'fees_name1' &&
                $obj->getCellByColumnAndRow(5, 11)->getValue() != 'amount1' &&
                $obj->getCellByColumnAndRow(6, 11)->getValue() != 'payment_type2' &&
                $obj->getCellByColumnAndRow(7, 11)->getValue() != 'fees_name2' &&
                $obj->getCellByColumnAndRow(8, 11)->getValue() != 'amount2' &&
                $obj->getCellByColumnAndRow(9, 11)->getValue() != 'payment_type3' &&
                $obj->getCellByColumnAndRow(10, 11)->getValue() != 'fees_name3' &&
                $obj->getCellByColumnAndRow(11, 11)->getValue() != 'amount3' ) {
    ...
    for (((($obj->getCellByColumnAndRow(0, $row)->getValue()) == 'session_id') ? $row = 12 : $row = 11); $row <= $rows; ++$row) {
            ...
            foreach ($Accounts as $key => $value) {
                ...
                //dd('value',$value);
                $not_mandatary = array('payment_type2','fees_name2','amount2','payment_type3','fees_name3','amount3');
                foreach($value as $keys => $val){
                    if(!in_array($keys, $not_mandatary)){
                        if(empty($val)){
                             if($ins_count!=0){
            ...
            foreach ($Accounts as $key => $val) {
               ...
               // }
                ...
                if (!$registration_exist_stu) {
                 ...
                 //insert sion_stufeestructure
            if ($val['registration_no']) {
                $class = \DB::table('class')->where('school_id', \Auth::user()->school_id)->where('id', $val['class_id'])->first();
                
                $clas=$class->class;
                $clas_id=$val['class_id'];
     ...
     public function doImportStudent($user, $request) {//change by mari 04.10.2017
        ...
        if ($obj->getCellByColumnAndRow(0, 1)->getValue() != 'session_id' &&
                ...
                $obj->getCellByColumnAndRow(15, 1)->getValue() != 'parent_contact_no' &&
                $obj->getCellByColumnAndRow(16, 1)->getValue() != 'parent_email' &&
                $obj->getCellByColumnAndRow(17, 1)->getValue() != 'student_type' &&
                $obj->getCellByColumnAndRow(18, 1)->getValue() != 'contact_no' &&
                $obj->getCellByColumnAndRow(19, 1)->getValue() != 'address' &&
                $obj->getCellByColumnAndRow(20, 1)->getValue() != 'bus_id'&&
            $obj->getCellByColumnAndRow(21, 1)->getValue() != 'aadhar_no'  &&//updated 9-5-2018 by priya
            $obj->getCellByColumnAndRow(22, 1)->getValue() != 'emis_no'  &&//updated 9-5-2018 by priya
            $obj->getCellByColumnAndRow(23, 1)->getValue() != 'rte_no' //updated 9-5-2018 by priya
        ...
        for (((($obj->getCellByColumnAndRow(0, $row)->getValue()) == 'session_id') ? $row = 2 : $row = 1); $row <= $rows; ++$row) {
            ...
            $Accounts[$Iterator] = array(
                ...
                'parent_contact_no' => $obj->getCellByColumnAndRow(15, $row)->getValue(),
                'parent_email' => $obj->getCellByColumnAndRow(16, $row)->getValue(),
                'student_type' => $obj->getCellByColumnAndRow(17, $row)->getValue(),
                'contact_no' => $obj->getCellByColumnAndRow(18, $row)->getValue(),
                'address' => $obj->getCellByColumnAndRow(19, $row)->getValue(),
                'bus_id' => $obj->getCellByColumnAndRow(20, $row)->getValue(),
                'aadhar_no' => $obj->getCellByColumnAndRow(21, $row)->getValue(),
                'emis_no' => $obj->getCellByColumnAndRow(22, $row)->getValue(),
                'rte_no' => $obj->getCellByColumnAndRow(23, $row)->getValue()
            ...
            foreach ($Accounts as $key => $value) {
                ...
                $not_mandatary = array('contact_no','address','bus_id','aadhar_no','emis_no','rte_no');
                foreach($value as $keys => $val){
                    if(!in_array($keys, $not_mandatary)){
                        if(empty($val)){
                             if($ins_count!=0){
           ...
           // dd($Accounts[1]['roll_no']);
            foreach ($Accounts as $key => $val) {
                ...
                if(!empty($val['aadhar_no']))
                {
                    ...
                    // Aadhar No Check
                    $aadhar_exist = \DB::table('student')->where('school_id', \Auth::user()->school_id)->where('aadhar_no', $val['aadhar_no'])->first();
                    if ($aadhar_exist)
                    {
                ...
                //Check EMIS(Educational Management Information System) No exist
                if(!empty($val['emis_no']))
                {
                    $emis_no_exist = Students::where('emis_no', $val['emis_no'])
                        ->where('school_id', \Auth::user()->school_id)
                        ->first();
                    if ($emis_no_exist)
                    {
                ...
                if (!$parent_exsit) {
                    ...
                    $parent_user_id = \DB::table('users')->insertGetId([
                        'type' => 'parent',
                        'school_id' => \Auth::user()->school_id,
                        'username' => $val['registration_no'] . "p",
                        'password' => \Hash::make($val['parent_contact_no']),
                        'hint_password' => $val['parent_contact_no']
                    ]);
                    if ($parent_user_id) {
                ...
                if ($user_id) {
                    ...
                    ->update(['username' => $val['registration_no'] . "s"]);
                }
                ...
                */
                ...
                $insert_stu = [
                    ...
                    'parent_id' => $pId,
                    'avatar' => 'student/default_avatar.png',
                    'aadhar_no' => $val['aadhar_no'],//updated 9-5-2018 by priya
                    'emi_no' => $val['emis_no'],//updated 9-5-2018 by priya
                    'rte' => $val['rte_no']//updated 9-5-2018 by priya
                ];
                $student_id = Students::insertGetId($insert_stu);
                $ins_count++;
                //send email
